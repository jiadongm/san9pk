...
async function loadData() {
  ...
  [state.officers, state.scenarios, state.availability, state.relationships, state.recommendations, state.manifest] = data;
}
...
function renderRecommendations() {
  ...
  const teams = state.recommendations.filter((item) => item.scenarioId === scenarioId && item.teamSize === teamSize).sort((a, b) => a.rank - b.rank);
  $("recommendation-note").textContent = `${scenario.title}（${scenario.year}年${scenario.month}月） · ${teamSize} 人高分候选。联携分不混入能力值；能力均值仅用于同分排序。`;
...
function initialiseListeners() {
  ...
  $("scenario-filter").addEventListener("change", renderRecommendations);
