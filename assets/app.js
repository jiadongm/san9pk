...
const affinityAnchors = [
  { label: "张角", position: 7 }, { label: "曹操", position: 25 }, { label: "刘备", position: 75 },
  { label: "袁绍", position: 101 }, { label: "孙权", position: 125 }, { label: "吕布", position: 145 },
];
...
function renderAffinityRing() {
  ...
  const anchors = affinityAnchors.map((anchor) => {
    const position = anchor.position;
    const angle = -Math.PI / 2 + (Math.PI * 2 * position) / 150;
    const x = (200 + Math.cos(angle) * 130).toFixed(2);
    const y = (200 + Math.sin(angle) * 130).toFixed(2);
    return `<circle class="affinity-anchor-marker" cx="${(200 + Math.cos(angle) * 151).toFixed(2)}" cy="${(200 + Math.sin(angle) * 151).toFixed(2)}" r="4" /><text class="affinity-anchor" x="${x}" y="${Number(y) + 4}">${anchor.label} ${position}</text>`;
  }).join("");
  ...
  $("affinity-ring").innerHTML = `<svg viewBox="0 0 400 400" role="group" aria-label="0 至 149 的相性圆环；具名标记为位置锚点，不表示势力归属"><circle class="affinity-guide" cx="200" cy="200" r="151" />${anchors}<circle class="affinity-ring-hit-target" cx="200" cy="200" r="151" />${points}${labels}<text class="affinity-center" x="200" y="190">相性位置</text><text class="affinity-value" x="200" y="220">${selected}</text></svg>`;
  ...
  $("affinity-ring").querySelector(".affinity-ring-hit-target").addEventListener("click", (event) => {
